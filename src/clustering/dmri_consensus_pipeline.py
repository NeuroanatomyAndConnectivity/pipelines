...
from clustering.cons_cluster import ConsCluster
from dmri_clustering_variables import subjects, workingdir, fsaverage, clusterdir, consensusdir, consensus_dg_template, consensus_dg_args, hemispheres, similarity_types, cluster_types, n_clusters
...
##Datagrabber for subjects##
    dg_subjects = pe.Node(nio.DataGrabber(infields=['fs','hemi', 'cluster', 'sim', 'n_clusters'], outfields=['all_subjects']), name="dg_subjects")
    ...
    wf.connect(fs_infosource, 'fs', dg_subjects, 'fs')
...
##Consensus between subjects##
    ...
    intersubject_cluster = pe.Node(ConsCluster(), name = 'intersubject_cluster')
