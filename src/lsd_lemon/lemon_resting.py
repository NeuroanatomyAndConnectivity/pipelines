...
def create_lemon_resting(subject, working_dir, data_dir, freesurfer_dir, out_dir,
    ...
    templates={'func': 'nifti/lemon_resting/rest.nii.gz',
               ...
               'mat_moc' : 'preprocessed/lemon_resting/realign/MAT/*',
               'moco_par' : 'preprocessed/lemon_resting/realign/rest_realigned.par',
               'fullwarp' : 'preprocessed/lemon_resting/coregister/transforms2anat/fullwarpfield.nii.gz',
               'epi2anat_dat': 'preprocessed/lemon_resting/coregister/transform2anat/rest2anat.dat',
               'unwarped_mean_epi2fmap': 'preprocessed/lemon_resting/coregister/rest_mean2fmap_unwarped.nii.gz',
               }
    ...
    func_preproc.connect([(selectfiles, remove_vol, [('func', 'in_file')]),
                          ...
                          (selectfiles, transform_ts, [('anat_head', 'inputnode.anat_head'),
                                                       ('mat_moco', 'inputnode.mat_moco'),
                                                       ('fullwarp', 'inputnode.fullwarp')]),
                          (selectfiles, denoise, [('func_mask', 'inputnode.brain_mask'),
                                                  ('anat_brain', 'inputnode.anat_brain'),
                                                  ('moco_par', 'inputnode.moco_par'),
                                                  ('epi2anat_dat', 'inputnode.epi2anat_dat'),
                                                  ('unwarped_mean_epi2fmap', 'inputnode.unwarped_mean'),
                                                  ('trans_ts', 'inputnode')]),
                          (transform_ts, denoise, [('outputnode.trans_ts','inputnode.epi_coreg')]),
                          (denoise, sink, [('outputnode.combined_motion','denoise.artefact.@combined_motion'),
                                           ...
                                           ('outputnode.unfiltered_file', '@unfiltered'),
                                           ('outputnode.unnormalized_file', '@unnormalized')
                                           ])
                          ])
